...
8. Complete native degree-5 genus-one covering construction for
   `FiveSelmerGroup(E)`.
9. Complete local solubility and relation computation for native 5-descent.
10. Complete native Cassels-pairing entry computation for 5-coverings.
11. Combine point-search or height-pairing lower bounds with Selmer upper bounds
...
`FiveSelmerGroup(E)` now has a native task scaffold, but native certificates
must not claim computed 5-Selmer groups until degree-5 covering construction,
local solubility, and relation computation are complete.  Cassels-pairing
certificates must not claim computed entries unless the matrix entries are
actually supplied or computed and verified alternating.
